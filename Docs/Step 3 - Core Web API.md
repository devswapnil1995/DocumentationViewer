...
> API versioning can be applied at both controller and action level. I generally prefer controller-level versioning when an entire API resource changes, because it keeps versions isolated. I use action-level mapping when only specific endpoints differ between versions
